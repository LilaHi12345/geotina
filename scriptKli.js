...
const loadQuizKli = async () => {
  const res = await fetch("quizKli.json");
  quizKli = await res.json();
  loadQuestion();
};

// Function to load the current question and options
const loadQuestion = () => {
  const questionObj = quizKli[currentQuestion];
...
document.getElementById("next-btn").addEventListener("click", () => {
  currentQuestion++;
  if (currentQuestion < quizKli.length) {
    loadQuestion();
    const progress = (currentQuestion / quizKli.length) * 100;
...
for (let i = 0; i < 4; i++) {
  document.getElementById(`btn${i}`).addEventListener("click", (event) => {
    ...
    if (
      quizKli[currentQuestion].options[i] === quizKli[currentQuestion].answer
    ) {
      score++;
      ...
      document.getElementById("message").innerText = "Pareiza atbilde ir " + quizKli[currentQuestion].answer;
    }
    for (let j = 0; j < 4; j++) {
