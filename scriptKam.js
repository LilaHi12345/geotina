...
const loadQuizKam = async () => {
  const res = await fetch("quizKam.json");
  quizKam = await res.json();
  loadQuestion();
};

// Function to load the current question and options
const loadQuestion = () => {
  const questionObj = quizKam[currentQuestion];
...
document.getElementById("next-btn").addEventListener("click", () => {
  currentQuestion++;
  if (currentQuestion < quizKam.length) {
    loadQuestion();
    const progress = (currentQuestion / quizKam.length) * 100;
...
for (let i = 0; i < 4; i++) {
  document.getElementById(`btn${i}`).addEventListener("click", (event) => {
    ...
    if (
      quizKam[currentQuestion].options[i] === quizKam[currentQuestion].answer
    ) {
      score++;
      ...
      document.getElementById("message").innerText = "Pareiza atbilde ir " + quizKam[currentQuestion].answer;
    }
    for (let j = 0; j < 4; j++) {
