...
const loadQuizVir = async () => {
  const res = await fetch("quizVir.json");
  quizVir = await res.json();
  loadQuestion();
};

// Function to load the current question and options
const loadQuestion = () => {
  const questionObj = quizVir[currentQuestion];
...
document.getElementById("next-btn").addEventListener("click", () => {
  currentQuestion++;
  if (currentQuestion < quizVir.length) {
    loadQuestion();
    const progress = (currentQuestion / quizVir.length) * 100;
...
for (let i = 0; i < 4; i++) {
  document.getElementById(`btn${i}`).addEventListener("click", (event) => {
    ...
    if (
      quizVir[currentQuestion].options[i] === quizVir[currentQuestion].answer
    ) {
      score++;
      ...
      document.getElementById("message").innerText = "Pareiza atbilde ir " + quizVir[currentQuestion].answer;
    }
    for (let j = 0; j < 4; j++) {
