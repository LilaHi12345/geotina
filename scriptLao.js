...
const loadQuizLao = async () => {
  const res = await fetch("quizLao.json");
  quizLao = await res.json();
  loadQuestion();
};

// Function to load the current question and options
const loadQuestion = () => {
  const questionObj = quizLao[currentQuestion];
...
document.getElementById("next-btn").addEventListener("click", () => {
  currentQuestion++;
  if (currentQuestion < quizLao.length) {
    loadQuestion();
    const progress = (currentQuestion / quizLao.length) * 100;
...
for (let i = 0; i < 4; i++) {
  document.getElementById(`btn${i}`).addEventListener("click", (event) => {
    ...
    if (
      quizLao[currentQuestion].options[i] === quizLao[currentQuestion].answer
    ) {
      score++;
      ...
      document.getElementById("message").innerText = "Pareiza atbilde ir " + quizLao[currentQuestion].answer;
    }
    for (let j = 0; j < 4; j++) {
