...
const loadQuizDom = async () => {
  const res = await fetch("quizDom.json");
  quizDom = await res.json();
  loadQuestion();
};

// Function to load the current question and options
const loadQuestion = () => {
  const questionObj = quizDom[currentQuestion];
...
document.getElementById("next-btn").addEventListener("click", () => {
  currentQuestion++;
  if (currentQuestion < quizDom.length) {
    loadQuestion();
    const progress = (currentQuestion / quizDom.length) * 100;
...
for (let i = 0; i < 4; i++) {
  document.getElementById(`btn${i}`).addEventListener("click", (event) => {
    ...
    if (
      quizDom[currentQuestion].options[i] === quizDom[currentQuestion].answer
    ) {
      score++;
      ...
      document.getElementById("message").innerText = "Pareiza atbilde ir " + quizDom[currentQuestion].answer;
    }
    for (let j = 0; j < 4; j++) {
