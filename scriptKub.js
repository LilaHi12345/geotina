...
const loadQuizKub = async () => {
  const res = await fetch("quizKub.json");
  quizKub = await res.json();
  loadQuestion();
};

// Function to load the current question and options
const loadQuestion = () => {
  const questionObj = quizKub[currentQuestion];
...
document.getElementById("next-btn").addEventListener("click", () => {
  currentQuestion++;
  if (currentQuestion < quizKub.length) {
    loadQuestion();
    const progress = (currentQuestion / quizKub.length) * 100;
...
for (let i = 0; i < 4; i++) {
  document.getElementById(`btn${i}`).addEventListener("click", (event) => {
    ...
    if (
      quizKub[currentQuestion].options[i] === quizKub[currentQuestion].answer
    ) {
      score++;
      ...
      document.getElementById("message").innerText = "Pareiza atbilde ir " + quizKub[currentQuestion].answer;
    }
    for (let j = 0; j < 4; j++) {
