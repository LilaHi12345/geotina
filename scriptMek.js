...
const loadQuizMek = async () => {
  const res = await fetch("quizMek.json");
  quizMek = await res.json();
  loadQuestion();
};

// Function to load the current question and options
const loadQuestion = () => {
  const questionObj = quizMek[currentQuestion];
...
document.getElementById("next-btn").addEventListener("click", () => {
  currentQuestion++;
  if (currentQuestion < quizMek.length) {
    loadQuestion();
    const progress = (currentQuestion / quizMek.length) * 100;
...
for (let i = 0; i < 4; i++) {
  document.getElementById(`btn${i}`).addEventListener("click", (event) => {
    ...
    if (
      quizMek[currentQuestion].options[i] === quizMek[currentQuestion].answer
    ) {
      score++;
      ...
      document.getElementById("message").innerText = "Pareiza atbilde ir " + quizMek[currentQuestion].answer;
    }
    for (let j = 0; j < 4; j++) {
