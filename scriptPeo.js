...
const loadQuizPeo = async () => {
  const res = await fetch("quizPeo.json");
  quizPeo = await res.json();
  loadQuestion();
};

// Function to load the current question and options
const loadQuestion = () => {
  const questionObj = quizPeo[currentQuestion];
...
document.getElementById("next-btn").addEventListener("click", () => {
  currentQuestion++;
  if (currentQuestion < quizPeo.length) {
    loadQuestion();
    const progress = (currentQuestion / quizPeo.length) * 100;
...
for (let i = 0; i < 4; i++) {
  document.getElementById(`btn${i}`).addEventListener("click", (event) => {
    ...
    if (
      quizPeo[currentQuestion].options[i] === quizPeo[currentQuestion].answer
    ) {
      score++;
      ...
      document.getElementById("message").innerText = "Pareiza atbilde ir " + quizPeo[currentQuestion].answer;
    }
    for (let j = 0; j < 4; j++) {
