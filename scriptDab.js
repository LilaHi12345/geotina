...
const loadQuizDab = async () => {
  const res = await fetch("quizDab.json");
  quizDab = await res.json();
  loadQuestion();
};

// Function to load the current question and options
const loadQuestion = () => {
  const questionObj = quizDab[currentQuestion];
...
document.getElementById("next-btn").addEventListener("click", () => {
  currentQuestion++;
  if (currentQuestion < quizDab.length) {
    loadQuestion();
    const progress = (currentQuestion / quizDab.length) * 100;
...
for (let i = 0; i < 4; i++) {
  document.getElementById(`btn${i}`).addEventListener("click", (event) => {
    ...
    if (
      quizDab[currentQuestion].options[i] === quizDab[currentQuestion].answer
    ) {
      score++;
