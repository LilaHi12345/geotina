...
const loadQuizKur = async () => {
  const res = await fetch("quizKur.json");
  quizKur = await res.json();
  loadQuestion();
};

// Function to load the current question and options
const loadQuestion = () => {
  const questionObj = quizKur[currentQuestion];
...
document.getElementById("next-btn").addEventListener("click", () => {
  currentQuestion++;
  if (currentQuestion < quizKur.length) {
    loadQuestion();
    const progress = (currentQuestion / quizKur.length) * 100;
...
for (let i = 0; i < 4; i++) {
  document.getElementById(`btn${i}`).addEventListener("click", (event) => {
    ...
    if (
      quizKur[currentQuestion].options[i] === quizKur[currentQuestion].answer
    ) {
      score++;
      ...
      document.getElementById("message").innerText = "Pareiza atbilde ir " + quizKur[currentQuestion].answer;
    }
    for (let j = 0; j < 4; j++) {
