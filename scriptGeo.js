...
const loadQuizGeo = async () => {
  const res = await fetch("quizGeo.json");
  quizGeo = await res.json();
  loadQuestion();
};

// Function to load the current question and options
const loadQuestion = () => {
  const questionObj = quizGeo[currentQuestion];
...
for (let i = 0; i < 4; i++) {
  document.getElementById(`btn${i}`).addEventListener("click", (event) => {
    ...
    if (
      quizGeo[currentQuestion].options[i] === quizGeo[currentQuestion].answer
    ) {
      score++;
      ...
      document.getElementById("message").innerText = "Pareiza atbilde ir " + quizGeo[currentQuestion].answer;
    }
    for (let j = 0; j < 4; j++) {
