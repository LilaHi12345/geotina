function apstiprinajums() {
  var komanda1 = document.getElementById('ievade1').value
  var komanda2 = document.getElementById('ievade2').value

  document.getElementById('komanda1').innerHTML=komanda1;
...
const loadQuizSim = async () => {
  const res = await fetch("quizSim.json");
  quizSim = await res.json();
  loadQuestion();
};

// Function to load the current question and options
const loadQuestion = () => {
  const questionObj = quizSim[currentQuestion];
...
document.getElementById("next-btn").addEventListener("click", () => {
  currentQuestion++;
  if (currentQuestion < quizSim.length) {
    loadQuestion();
    const progress = (currentQuestion / quizSim.length) * 100;
...
for (let i = 0; i < 4; i++) {
  document.getElementById(`btn${i}`).addEventListener("click", (event) => {
    ...
    if (
      quizSim[currentQuestion].options[i] === quizSim[currentQuestion].answer
    ) {
      score++;
      ...
      document.getElementById("message").innerText = "Pareiza atbilde ir " + quizSim[currentQuestion].answer;
    }
    for (let j = 0; j < 4; j++) {
